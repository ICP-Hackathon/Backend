...
from DB import schemas, utils, users
...
@router.get("/{user_address}", response_model=schemas.User)
...
def add_user(user: schemas.User, db: Session = Depends(utils.get_db)):
...
def update_user(user: schemas.User, db: Session = Depends(utils.get_db)):
